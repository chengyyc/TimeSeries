# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 10:57:41 2020
...
#train
simpleExpSmooth_model = SimpleExpSmoothing(row_now[name].values).fit(smoothing_level=alpha,optimized=False)
#predict
predictions[id_name_new] = simpleExpSmooth_model.forecast(28)#.values
#evaluate
score = sqrt(simpleExpSmooth_model.sse/1913)
...
predictions_new.to_csv("C:/Users/yingy/Desktop/research/Kaggle - M5 Forecasting/data/some results/Simple Exponential Smoothing/point_prediction_validation.csv", index=True)
stats_new = stats.copy()
stats_new.to_csv("C:/Users/yingy/Desktop/research/Kaggle - M5 Forecasting/data/some results/Simple Exponential Smoothing/RMSE_validation.csv", index=False)
...
for i in list(range(42840)):
    ...
    id_name_new = id_name_new.strip('\']')
    simpleExpSmooth_model = SimpleExpSmoothing(row_now[name].values).fit(smoothing_level=alpha,optimized=False)
    predictions_e[id_name_new] = simpleExpSmooth_model.forecast(28)#.values
    score = sqrt(simpleExpSmooth_model.sse/1913)
    stats_e = stats.append({'ID Name':id_name_new, 'RMSE':score},ignore_index=True)
    print(i)

predictions_new = predictions_e.copy()
predictions_new = predictions_new.T
predictions_new.to_csv("C:/Users/yingy/Desktop/research/Kaggle - M5 Forecasting/data/some results/Simple Exponential Smoothing/point_prediction_evaluation.csv", index=True)
stats_new = stats_e.copy()
stats_new.to_csv("C:/Users/yingy/Desktop/research/Kaggle - M5 Forecasting/data/some results/Simple Exponential Smoothing/RMSE_evaluation.csv", index=False)
